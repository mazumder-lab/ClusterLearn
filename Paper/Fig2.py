...
PREFIX = "../ClusterLearn/results/results_fig_2"
...
N = [80, 100, 150, 200, 250, 300, 400, 500, 600, 800, 1000]
...
beta = []
n_non_l = 3
for l in [[-2]*4 + [0]*(levels-8) + [2]*4 for j in range(n_non_l)]:
    beta += l
...
for counter_run in range(N_run):    
    for n in N:
        ...
        beta_bcd, time_bcd = BCD_wrapper(X_cat0_train, y, X_cat0_val, y_val,numlevels_=[levels]*n_cat,l0_list=[0],l1_list=np.flip(lambda0))
